...
The following code calculates the Confidence Interval (CI) for the two values based on normal distribution.
```{r}
conf_int =
  boot_straps %>%
  pivot_longer(
...
# Problem 3
Read and tidy the data. Since there are no missing values nor known values, my main focus is to reformat some variables based on the description.
```{r}
rm(list = ls())
...
To compare different models, use cross validation (CV). In this case, we choose 10-fold CV since the dataset is large enough. Calculate the rmse for each test on different models:
